...
fun findBiggestNumberAndCountIt(numberList: List<Int>): Int {
    // 1. groesste zahl finden
    var biggestNr: Int = 0
    var i: Int = 0
    var biggestNrCounter = 0
    var newBiggestNumberFound = false
    while (i < numberList.size) {
        // Logik, um groesste Zahl festzustellen
        // Vergleich: aktuelle Zahl mit biggestNr
        if (biggestNr <= numberList[i]) {
            if (biggestNr < numberList[i]) {
                // wenn biggestNr kleiner ist, stattdessen die aktuelle Zahl reinschreiben
                biggestNr = numberList[i]
                newBiggestNumberFound = true
                if (newBiggestNumberFound){
                    biggestNrCounter = 1
                }
            } else if (biggestNr == numberList[i]){
                biggestNrCounter++
            }
    ...
    println("Die groesste Zahl ist $biggestNr und sie kommt $biggestNrCounter mal vor.")
    return biggestNr
}
