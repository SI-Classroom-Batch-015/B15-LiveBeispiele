...
fun main() {
    ...
    println(getPrice(filmArtikelNr[0]))
    println(getPrice(filmArtikelNr[1]))
    println(getPrice(filmArtikelNr[2]))
    println(getPrice(0x1A3FE))
}

// TODO Exception Handling einbauen
fun getPrice(artikelNr: Int): Double {
    // 1. titel über die nr identifizieren
    val titel = getTitel(artikelNr)
    // 2. preis über den titel identifizieren
    val preis = preisListe[titel]!!
